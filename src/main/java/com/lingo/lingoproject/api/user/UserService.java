...
import com.lingo.lingoproject.db.domain.MemberShipLog;
...
import com.lingo.lingoproject.db.domain.UserPoint;
import com.lingo.lingoproject.db.domain.Withdrawer;
import com.lingo.lingoproject.db.domain.enums.Drinking;
import com.lingo.lingoproject.db.domain.enums.MemberShipType;
import com.lingo.lingoproject.db.domain.enums.Religion;
import com.lingo.lingoproject.db.domain.enums.Role;
import com.lingo.lingoproject.db.domain.enums.SignupStatus;
import com.lingo.lingoproject.db.domain.enums.Smoking;
...
import com.lingo.lingoproject.db.repository.ChatroomParticipantRepository;
import com.lingo.lingoproject.db.repository.DormantAccountRepository;
import com.lingo.lingoproject.db.repository.FcmTokenRepository;
import com.lingo.lingoproject.db.repository.FriendInvitationLogRepository;
...
import com.lingo.lingoproject.db.repository.MemberShipLogRepository;
...
import com.lingo.lingoproject.db.repository.UserPointRepository;
import com.lingo.lingoproject.db.repository.UserRepository;
import com.lingo.lingoproject.db.repository.WithdrawerRepository;
import com.lingo.lingoproject.common.security.TokenType;
import com.lingo.lingoproject.api.user.dto.Address;
...
public class UserService {
  ...
  private static final int MAX_FRIEND_INVITATION_CODE_INPUT_ATTEMPTS = 5;
  ...
  public void saveUserInfo(SignupUserInfoDto dto){
    ...
    User user = userRepository.findById(dto.id()).orElseThrow(
        () -> new RingoException("해당 회원을 찾을 수 없습니다.", ErrorCode.NOT_FOUND_USER, HttpStatus.BAD_REQUEST)
    );
    ...
    List<Hashtag> hashtags = getUserHashtags(dto, user);
...
//    }
  ...
  private List<Hashtag> getUserHashtags(SignupUserInfoDto dto, User user){
  ...
  public void logout(HttpServletRequest request){
    ...
    if (!accessToken.startsWith(ACCESS_TOKEN_PREFIX)) throw new RingoException("토큰이 잘못전달되었습니다.", ErrorCode.BAD_REQUEST, HttpStatus.BAD_REQUEST);
  ...
  public void checkFriendInvitationCodeAndProvideReward(User user, String code) {
    ...
    if (friendInvitationLogRepository.getNumberOfParticipation(user.getId(), true) > 0 ||
        friendInvitationLogRepository.getNumberOfParticipation(user.getId(), false)
            > MAX_FRIEND_INVITATION_CODE_INPUT_ATTEMPTS) {

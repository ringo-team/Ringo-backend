...
public interface PostRepository extends JpaRepository<Post, Long> {
  ...
  @Query("select p from Post p left join p.place q where (:placeId is null or q.id = :placeId) and (:category is null or p.category = :category) order by p.createdAt desc")
  Page<Post> findByPlaceAndCategoryOrderByDesc(Long placeId, PostCategory category, Pageable pageable);

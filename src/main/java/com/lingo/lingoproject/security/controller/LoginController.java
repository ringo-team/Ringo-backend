...
public class LoginController {
  ...
  public ResponseEntity<ResultMessageResponseDto> verifyDuplicatedLoginId(@RequestParam(value = "email") String email)
  {
    ...
    if (isDuplicated){
      return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body(new ResultMessageResponseDto("중복된 아이디입니다."));
    }
    return ResponseEntity.status(HttpStatus.OK).body(new ResultMessageResponseDto("사용가능한 아이디 입니다."));
  }
